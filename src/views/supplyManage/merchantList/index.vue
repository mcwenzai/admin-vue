      ...
      <el-form :model="queryParams" :inline="true">
        <el-col :lg="8" :md="12" :xs="24" :sm="24">
          <el-form-item label="企业名称:">
            <el-input style="width: 250px;" placeholder="请输入厂商名称" v-model="queryParams.companyName"></el-input>
          </el-form-item>
        </el-col>
        <el-col :lg="8" :md="12" :xs="24" :sm="24">
          <el-form-item label="审批状态:">
            <el-select
              v-model="queryParams.ban"
            >
              <el-option
        ...
        <el-col :lg="8" :md="12" :xs="24" :sm="24">
          <el-form-item label="所在地区:">
            <place-cascader :place="queryParams.place" @getPlaceInfo="getPlaceInfo"></place-cascader>
          </el-form-item>
        </el-col>
      </el-form>
    </el-row>
    ...
    <el-row style="padding: 20px 0">
      <sht-table :tableData="tableData" :tableColumn="tableColumn" v-loading="loading" @handleSelectionChange="handleSelectionChange">
        <template slot="companyName" slot-scope="scope">
          <el-button type="text" @click="handleToDetail(scope.scope)">{{scope.scope.companyName}}</el-button>
        </template>
        <template slot="logoUrl" slot-scope="scope">
          <el-image style="width: 100px; height: 100px" fit="fill" :src="scope.scope.logoUrl" :preview-src-list="[scope.scope.logoUrl]"></el-image>
        </template>
        <template slot="operate" slot-scope="scope">
          <el-button type="text" @click="handleToDetail(scope.scope)" v-if="scope.scope.ban === '正常'">查看</el-button>
          <el-button type="text" @click="handleFreezeOperate(scope.scope, true)" v-if="scope.scope.ban === '正常'">冻结</el-button>
          <el-button type="text" @click="handleFreezeOperate(scope.scope, false)" v-if="scope.scope.ban === '冻结'">解冻</el-button>
          <el-button type="text" @click="handleToDetail(scope.scope)" v-if="scope.scope.ban === '断续'">续约</el-button>
        </template>
      </sht-table>
    <pagination 
      v-show="total > 0" 
...
export default {
  ...
  methods: {
    ...
    handleToDetail(value) {
      this.$router.push({
        path: '/supplyManage/merchantList/detail',
        query: {
          id: value.id
        }
      })
